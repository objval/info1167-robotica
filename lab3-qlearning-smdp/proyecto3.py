...
class AgenteQLearning:
    ...
    def tiempo_accion(self, a):
        ...
        mu, sigma = {'Norte':(2,.2),'Sur':(2,.2),'Este':(3,.3),'Oeste':(3,.3)}[a]
...
def main():
    ...
    slider_alpha = Slider(sx, sy, 230, 14, 0.01, 0.5, 0.10, "Alpha (aprendizaje)", "{:.2f}")

    sy += 42

    slider_epsilon = Slider(sx, sy, 230, 14, 0.01, 1.0, 0.30, "Epsilon (exploracion)", "{:.2f}")

    sy += 42

    slider_velocidad = Slider(sx, sy, 230, 14, 100, 800, 350, "Velocidad (ms)", "{:.0f}")
    ...
    widgets = [slider_gamma, slider_alpha, slider_epsilon, slider_velocidad,

               toggle_valores, toggle_flechas, btn_entreno, btn_auto, btn_reset, btn_animar]
    ...
    while corriendo:
        ...
        if entrenando and agente.episodios < meta_entreno:
            ...
            for _ in range(20):
                ...
                if agente.episodios < meta_entreno:
                    ...
                    agente.entrenar_episodio(slider_gamma.valor, slider_alpha.valor, eps)

                else:

                    break

            if meta_entreno > 40000 and agente.episodios > 300 and len(agente.recompensas) > 60:
        ...
        pygame.draw.rect(pantalla, C['panel'], (ANCHO_GRILLA + 2, 0, PANEL_LATERAL, ALTO))

        pygame.draw.line(pantalla, C['borde'], (ANCHO_GRILLA + 2, 0), (ANCHO_GRILLA + 2, ALTO), 1)

        dibujar_header(pantalla, f"Proyecto #3 — Q-Learning SMDP (alpha={slider_alpha.valor:.2f})")

        val = agente.valores()

        pol = agente.politica()

        dibujar_grilla(pantalla, val, pol, robot, rastro, pos_ini,

                       toggle_valores.activo, toggle_flechas.activo)
        ...
        for label, valor in [("Episodios", str(agente.episodios)),
        ...
        if entrenando:
            ...
            if prog > 0:
                ...
                pygame.draw.rect(pantalla, C['barra_fg'], (sx, y, int(bw * prog), 12), border_radius=3)

            pantalla.blit(f.render(f"{agente.episodios} ep", True, C['texto']), (sx, y + 15))
        ...
        dibujar_footer(pantalla, "ENTER=Entrenar  T=Auto  A=Animar  R=Reset  Q=Salir  |  Sliders ajustables")
