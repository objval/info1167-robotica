        ...
        for s in estados:
            ...
            for acc in ACTIONS:
                ...
                if modo == 'SMDP':
                    ...
                    mu, sigma = {'Norte':(2,.2),'Sur':(2,.2),'Este':(3,.3),'Oeste':(3,.3)}[acc]

                    desc = np.mean([gamma ** max(0.1, np.random.normal(mu, sigma)) for _ in range(20)])
...
def main():
    ...
    widgets = [slider_gamma, slider_velocidad, toggle_valores, toggle_flechas,

               btn_mdp, btn_smdp, btn_reset]
    ...
    while corriendo:
                ...
                if accion and accion != 'META':
                    ...
                    if nueva != robot:
                        ...
                        if modo == 'SMDP':
                            ...
                            mu, sigma = {'Norte':(2,.2),'Sur':(2,.2),'Este':(3,.3),'Oeste':(3,.3)}[accion]
        ...
        dibujar_header(pantalla, f"Proyecto #2 — {'MDP' if modo == 'MDP' else 'SMDP'} (gamma={slider_gamma.valor:.2f})")

        dibujar_grilla(pantalla, V_cache, pi_cache, robot, rastro, pos_ini,
        ...
        for label, val in [("Pasos", str(pasos)), ("Tiempo", f"{tiempo:.1f}s"),

                           ("P exito", "90%"), ("Inicio", str(pos_ini))]:
        ...
        if modo == 'SMDP':
            ...
            for linea in ["Tiempos (SMDP):", "  N/S ~ N(2, 0.2)", "  E/O ~ N(3, 0.3)", "  descuento = gamma^tau"]:

                pantalla.blit(f.render(linea, True, C['dim']), (sx, y))

                y += 16

        dibujar_footer(pantalla, "1=MDP  2=SMDP  ESPACIO=Pausar  R=Reset  Q=Salir  |  Sliders ajustables")
